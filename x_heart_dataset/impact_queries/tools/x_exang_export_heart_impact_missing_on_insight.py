...
def missing_data_attr(db, size, percentage, seed):
    ...
    modified = dropout(data, size, percentage, seed)
    new_df = pd.DataFrame(modified)  # Change % of data to NA in the dataset
    columns = ['sex', 'fbs', 'restecg', 'exang', 'slope', 'thal','num']
    new_df.columns = columns
    return new_df
...
table_name = 'heart'
connection = pg.connect("dbname=aheart_dataset_random_predicate_exang user=postgres password=zenvisage")
...
engine = create_engine(r'postgresql+psycopg2://postgres:zenvisage@localhost:5432/aheart_dataset_random_predicate_exang')
...
for i in mlist:
    for j in range(100):
        ...
        # df_attr = df_attr.drop('cp', 1)
        # df_attr_missing = missing_data_attr(df_attr, size, i, j)
        # df_attr_missing['cp'] = df['cp'].values
        ...
        df_a_m = df_a_m.drop('exang', 1)
        df_a_m_missing = missing_data(df_a_m, size, i, j)
        df_a_m_missing['exang'] = df['exang'].values
