...
def missing_data(db, N_samples, alpha, seed, percent):
    ...
    s3 = df['s3'].values
    s3 = dropout(s3, percent*rank[0], seed)

    s4 = df['s4'].values
    s4 = dropout(s4, percent*rank[1], seed)


    bmi = df['bmi'].values
    bmi = dropout(bmi, percent*rank[2], seed)

    s2 = df['s2'].values
    s2 = dropout(s2, percent*rank[3], seed)

    s1 = df['s1'].values
    s1 = dropout(s1, percent*rank[4], seed)

    s5 = df['s5'].values
    s5 = dropout(s5, percent*rank[5], seed)

    bp = df['bp'].values
    bp = dropout(bp, percent*rank[6], seed)

    s6 = df['s6'].values
    s6 = dropout(s6, percent*rank[7], seed)

    sex = df['sex'].values
    sex = dropout(sex, percent*rank[8], seed)

    age = df['age'].values
    age = dropout(age, percent*rank[9], seed)
  ...
  for k in k_list:
    ...
    for i in mlist:
      for j in range(100):
        ...
        temp_topk = ag.generate_skewness_insights(data)
        missing_topk = temp_topk[:k]
        with open('results/missing_vs_ideal_zipf_20.csv', 'a', newline='') as f:
