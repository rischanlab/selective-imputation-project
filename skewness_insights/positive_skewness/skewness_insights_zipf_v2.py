...
def missing_data(db, N_samples, alpha, seed):
    ...
    s3 = df['s3'].values
    s3 = dropout(s3, rank[0], seed)

    s4 = df['s4'].values
    s4 = dropout(s4, rank[1], seed)


    bmi = df['bmi'].values
    bmi = dropout(bmi, rank[2], seed)

    s2 = df['s2'].values
    s2 = dropout(s2, rank[3], seed)

    s1 = df['s1'].values
    s1 = dropout(s1, rank[4], seed)

    s5 = df['s5'].values
    s5 = dropout(s5, rank[5], seed)

    bp = df['bp'].values
    bp = dropout(bp, rank[6], seed)

    s6 = df['s6'].values
    s6 = dropout(s6, rank[7], seed)

    sex = df['sex'].values
    sex = dropout(sex, rank[8], seed)

    age = df['age'].values
    age = dropout(age, rank[9], seed)
...
for a in a_list:
  for k in k_list:
    ...
    for j in range(100):
        ...
        with open('results/v2_missing_vs_ideal_zipf_20.csv', 'a', newline='') as f:
