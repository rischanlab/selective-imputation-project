...
def missing_data(db, N_samples, alpha, seed):
    ...
    bmi = df['bmi'].values
    bmi = dropout(bmi, rank[8], seed)

    bp = df['bp'].values
    bp = dropout(bp, rank[7], seed)

    s1 = df['s1'].values
    s1 = dropout(s1, rank[6], seed)

    s2 = df['s2'].values
    s2 = dropout(s2, rank[5], seed)

    s3 = df['s3'].values
    s3 = dropout(s3, rank[4], seed)

    s4 = df['s4'].values
    s4 = dropout(s4, rank[3], seed)

    s5 = df['s5'].values
    s5 = dropout(s5, rank[2], seed)
