...
for k in k_list:
	topk = ag.generate_correlation_insights(db_ideal,k)
	for i in mlist:
	    for j in range(100):
	    	data = missing_data(df, i, j)
	    	missing_topk = ag.generate_correlation_insights(data,k)
	    	with open('results/cum_missing_vs_ideal.csv', 'a', newline='') as f:
