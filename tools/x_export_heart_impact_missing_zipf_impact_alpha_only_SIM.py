...
def missing_data_attr(db, N_samples, alpha, seed):
    ...
    cp = df['cp'].values
    cp = dropout(cp, rank[6], seed)

    thal = df['thal'].values
    thal = dropout(thal, rank[5], seed)

    slope = df['slope'].values
    slope = dropout(slope, rank[4], seed)

    exang = df['exang'].values
    exang = dropout(exang, rank[3], seed)
    
    restecg = df['restecg'].values
    restecg = dropout(restecg, rank[2], seed)

    sex = df['sex'].values
    sex = dropout(sex, rank[1], seed)

    fbs = df['fbs'].values
    fbs = dropout(fbs, rank[0], seed)
...
def missing_data_measure(db, N_samples, alpha, seed):
    ...
    age = df['age'].values
    age = dropout(age, rank[5], seed)

    restbp = df['restbp'].values
    restbp = dropout(restbp, rank[4], seed)

    chol = df['chol'].values
    chol = dropout(chol, rank[3], seed)

    thalach = df['thalach'].values
    thalach = dropout(thalach, rank[2], seed)

    ca = df['ca'].values
    ca = dropout(ca, rank[1], seed)

    oldpeak = df['oldpeak'].values
    oldpeak = dropout(oldpeak, rank[0], seed)
...
table_name = 'heart'
connection = pg.connect("dbname=heart_dataset_zipf_impact_alpha_sim user=postgres password=zenvisage")
