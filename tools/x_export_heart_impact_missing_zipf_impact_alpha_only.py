...
def missing_data_attr(db, N_samples, alpha, seed):
    col = 7
    rank = percentage_missing_zipf(col, N_samples, alpha, seed)
    df = db.copy()

    cp = df['cp'].values
    cp = dropout(cp, rank[0], seed)

    thal = df['thal'].values
    thal = dropout(thal, rank[1], seed)

    slope = df['slope'].values
    slope = dropout(slope, rank[2], seed)

    exang = df['exang'].values
    exang = dropout(exang, rank[3], seed)
    
    restecg = df['restecg'].values
    restecg = dropout(restecg, rank[4], seed)

    sex = df['sex'].values
    sex = dropout(sex, rank[5], seed)

    fbs = df['fbs'].values
    fbs = dropout(fbs, rank[6], seed)


    #num = df['num'].values
    #readmitted = dropout(readmitted, rank[7], seed)
...
def missing_data_measure(db, N_samples, alpha, seed):
    col = 6
    rank = percentage_missing_zipf(col, N_samples, alpha, seed)
    df = db.copy()

    age = df['age'].values
    age = dropout(age, rank[0], seed)

    restbp = df['restbp'].values
    restbp = dropout(restbp, rank[1], seed)

    chol = df['chol'].values
    chol = dropout(chol, rank[2], seed)

    thalach = df['thalach'].values
    thalach = dropout(thalach, rank[3], seed)

    ca = df['ca'].values
    ca = dropout(ca, rank[4], seed)

    oldpeak = df['oldpeak'].values
    oldpeak = dropout(oldpeak, rank[5], seed)
...
table_name = 'heart'
connection = pg.connect("dbname=heart_dataset_zipf_impact_alpha user=postgres password=zenvisage")
...
for a in a_list:
    for j in range(100):
        ...
        y = int(a * 100)
        table_name1 = "db_zipf" + str(y) + "_missing_attr" + str(j+1)
        table_name2 = "db_zipf" + str(y) + "_missing_measure" + str(j + 1)
