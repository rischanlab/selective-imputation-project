...
db_ideal = df.copy()
df_ideal = ag.skew_ideal(db_ideal)
...
for k in k_list:
    topk = ag.get_sum_skew_ideal(k,df_ideal)
    for i in mlist:
        for j in range(100):
            data = missing_data(df, i, j)
            missing_topk = ag.get_sum_skew_insights_cum(k, df_ideal, data)
            with open('results/cum_skewness_missing_vs_ideal_random.csv', 'a', newline='') as f:
