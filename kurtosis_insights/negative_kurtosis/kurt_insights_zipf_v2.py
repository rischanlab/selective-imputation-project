...
def missing_data(db, N_samples, alpha, seed):
    ...
    s4 = df['s4'].values
    s4 = dropout(s4, rank[7], seed)


    bmi = df['bmi'].values
    bmi = dropout(bmi, rank[4], seed)

    s2 = df['s2'].values
    s2 = dropout(s2, rank[8], seed)

    s1 = df['s1'].values
    s1 = dropout(s1, rank[5], seed)

    s5 = df['s5'].values
    s5 = dropout(s5, rank[3], seed)

    bp = df['bp'].values
    bp = dropout(bp, rank[2], seed)

    s6 = df['s6'].values
    s6 = dropout(s6, rank[6], seed)

    sex = df['sex'].values
    sex = dropout(sex, rank[0], seed)

    age = df['age'].values
    age = dropout(age, rank[1], seed)
...
columns = ['sex', 'age', 'bp', 's5', 'bmi', 's1', 's6', 's4', 's2', 's3']
