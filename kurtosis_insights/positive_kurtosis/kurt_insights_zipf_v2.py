...
def missing_data(db, N_samples, alpha, seed):
    ...
    s4 = df['s4'].values
    s4 = dropout(s4, rank[2], seed)


    bmi = df['bmi'].values
    bmi = dropout(bmi, rank[5], seed)

    s2 = df['s2'].values
    s2 = dropout(s2, rank[1], seed)

    s1 = df['s1'].values
    s1 = dropout(s1, rank[4], seed)

    s5 = df['s5'].values
    s5 = dropout(s5, rank[6], seed)

    bp = df['bp'].values
    bp = dropout(bp, rank[7], seed)

    s6 = df['s6'].values
    s6 = dropout(s6, rank[3], seed)

    sex = df['sex'].values
    sex = dropout(sex, rank[9], seed)

    age = df['age'].values
    age = dropout(age, rank[8], seed)
...
columns = ['s3', 's2', 's4', 's6', 's1', 'bmi', 's5', 'bp', 'age', 'sex']
...
for a in a_list:
  for k in k_list:
    ...
    for j in range(100):
        ...
        temp_topk = ag.generate_kurtosis_insights(data)
