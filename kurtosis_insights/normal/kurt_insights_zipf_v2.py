...
def missing_data(db, N_samples, alpha, seed):
    ...
    s3 = df['s3'].values
    s3 = dropout(s3, rank[8], seed)

    s4 = df['s4'].values
    s4 = dropout(s4, rank[4], seed)


    bmi = df['bmi'].values
    bmi = dropout(bmi, rank[0], seed)

    s2 = df['s2'].values
    s2 = dropout(s2, rank[6], seed)

    s1 = df['s1'].values
    s1 = dropout(s1, rank[2], seed)

    s5 = df['s5'].values
    s5 = dropout(s5, rank[1], seed)

    bp = df['bp'].values
    bp = dropout(bp, rank[5], seed)
...
columns = ['bmi', 's5', 's1', 's6', 's4', 'bp', 's2', 'age', 's3', 'sex']
...
for a in a_list:
  for k in k_list:
    topk = ag.generate_kurtosis_insights(db_ideal, k)
    for j in range(100):
        data = missing_data(df, N_samples, a, j)
        #print(data)

        missing_topk = ag.generate_kurtosis_insights(data, k)
