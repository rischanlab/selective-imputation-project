...
db_ideal = df.copy()
df_ideal = ag.kurt_ideal_abs(db_ideal)
...
for k in k_list:
    topk = ag.get_sum_kurt_ideal_abs(k,df_ideal)
    #print(topk)
    for i in mlist:
        for j in range(100):
            data = missing_data(df, i, j)
            data = missing_data(df, i, j)
            missing_topk = ag.get_sum_kurt_insights_cum_abs(k, df_ideal, data)
            with open('results/cum_kurtosis_missing_vs_ideal_abs.csv', 'a', newline='') as f:
